#Redoupshack 
 ...
 def logop(z): 
   for word in z + '\n': 
      ...
      time.sleep(0.01) 
 def maogit(): 
   os.system("xdg-open https://github.com/Red.oupshack/") 
 def ext(): 
 ...
 def historylog(): 
   ...
   print(manuhome) 
   for num , mail in enumerate(read): 
     print("{acl}[ {gcl}{Redooups} {acl}] {ycl}".format(mao=num+1,acl=acl,gcl=gcl,ycl=ycl)+mail["email"]) 
 ...
 def main2(): 
   try: 
     ...
     {acl}[{gcl} 5{bcl}.{acl} ]{ycl} GITHUB {acl}[{bcl} Redoupshack {acl}] 
 ...
 def main(): 
   try: 
     ...
     {acl}[{gcl} 4{bcl}.{acl} ]{ycl} GITHUB {acl}[{bcl} Redoupstrickeur {acl}] 
     {acl}[{rcl} 0{bcl}.{acl} ]{rcl} EXIT{acl} [{rcl} ××× {acl}] 
     """.format(acl=acl,bcl=bcl,gcl=gcl,ycl=ycl,rcl=rcl,ccl=ccl) 
     while True: 
